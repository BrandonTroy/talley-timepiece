...
app.time_since_last_ping = lambda: (datetime.now() - app.last_ping).total_seconds()
...
def index():
    return render_template('index.html', timezones=app.timezones, current_timezone=app.timezone, 
                            last_ping=app.time_since_last_ping(), sounds=app.alarm_sounds, current_sound=app.alarm_sound,
...
# receies update from pi when alarm starts going off
@app.route('/set-going-off', methods=['POST'])
def go_off():
    app.going_off = True
    app.snooze = False
    app.is_snoozed = False
...
# receives update from pi when alarm is snoozed
@app.route('/set-snoozed', methods=['POST'])
def set_snoozed():
    app.is_snoozed =  True
    return ''


# receives update from pi when alarm is stopped
@app.route('/set-stopped', methods=['POST'])
def set_stopped():
    app.going_off = False
    return ''
...
def api_client():
    return dumps({
        'timezone': app.timezone,
        'alarms': app.alarms,
        'last_ping': app.time_since_last_ping(),
